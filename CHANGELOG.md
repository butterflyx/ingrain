...
- `tsconfig.json`'s `lib` setting was missing `ES2020`, which
  `String.prototype.matchAll` (used throughout the parser) requires.
  Locally this was masked by `@types/node` transitively pulling the
  right lib declarations in as a side effect, so it only surfaced as
  `any`-typed values in a stricter external type-check.

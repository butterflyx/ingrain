...
Spaced repetition for [Obsidian](https://obsidian.md) that turns the notes
...
frontmatter. Ingrain works with the way you already take notes:
...
Ingrain isn't in the Community Plugins directory yet, so for now you build
it from source:
...
Ingrain only looks at notes carrying your configured deck tag — `#flashcards`
...
Ingrain ships a default look for `[!card]` callouts — a graduation-cap
...
that syntax is a real Obsidian footnote reference, and Ingrain won't
touch it there.)
...
Ingrain re-scans a note automatically when you create or edit it, and
